...
def batched_gather(data, inds, dim=0, no_batch_dims=0):
    ...
    # Check if dealing with numpy arrays
    ...
    for i, s in enumerate(data.shape[:no_batch_dims]):
        if is_numpy:
            ...
            new_shape = [1] * i + [-1] + [1] * (len(inds.shape) - i - 1)
    ...
    # Convert inds to numpy if needed
    if is_numpy:
        if isinstance(inds, torch.Tensor):
            inds = inds.numpy().astype(np.int64)
    ...
    # Convert ranges to tuple for proper numpy indexing
